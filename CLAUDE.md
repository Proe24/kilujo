...
5. **Barotrauma guide lives at `public/guides/barotrauma/`.** Treat those files as a self-contained static drop. Don't rewrite their styling to match the main site — the dark submarine theme is intentional once you click into the guide.
   - Global navigation (sticky topbar + responsive sidebar + auto prev/next) is handled by **`baro_nav.js`**. That file is the single source of truth for the page list, categories, and load order. Edit the `PAGES` array there to add or rename a mod — every page picks it up automatically.
   - The sidebar is **persistent on viewports ≥ 1280px** and **slides in from the right below that** (toggled by the "All mods" button or `/`). Breakpoint lives in the CSS injected by `baro_nav.js`.
